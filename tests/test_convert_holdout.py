...
class TestConvertHoldout(TestCase):
    def test_preprocess_csv(self):
        ...
        self.assertEqual(node_types, {1: "TYPE_FULL_NAME UP",
                                      2: "LOCAL UP",
                                      3: "LOCAL DOWN",
                                      4: "NAME DOWN",
                                      5: "NAME UP",
        ...
        self.assertEqual(paths, {1: [1, 2, 3, 4],
                                 2: [5, 2, 3, 6]}
                         )
    ...
    def test_convert_holdout(self):
        ...
        type_counter = Counter({'LOCAL UP': 1,
                                'LOCAL DOWN': 2,
                                'TYPE_FULL_NAME UP': 3,
                                'NAME DOWN': 4,
                                'NAME UP': 5,
        ...
        convert_holdout(
            holdout_data_path, holdout_output_folder, vocab, config, cpu_count(), split_context,
        ...
        with open(path.join(holdout_output_folder, "buffered_paths_0.pkl"), "rb") as buf_paths:
            ...
            from_tokens = np.array([[10, 8, 7, 10, 10],
                                   [2, 2, 2, 2, 2],
                                   [2, 2, 2, 2, 2],
                                   [2, 2, 2, 2, 2],
                                   [2, 2, 2, 2, 2]])
            to_tokens = np.array([[9, 10, 10, 6, 4],
                                 [2, 2, 2, 5, 2],
                                 [2, 2, 2, 2, 2],
                                 [2, 2, 2, 2, 2],
                                 [2, 2, 2, 2, 2]])
            path_types = np.array([[0, 0, 0, 0, 0],
                                   [7, 5, 5, 7, 7],
                                   [9, 9, 9, 9, 9],
                                   [8, 8, 8, 8, 8],
                                   [6, 4, 4, 6, 6],
